...
export default function getExerciseImage(solution: string, excercise_number: string, page_number: string, book_name: string): Promise<Buffer | null> {
    return new Promise(async (resolve) => {
        ...
        decoded_solution = `<style>html * {font-size: 14px;${decoded_solution.includes('class="math') ? '' : 'background: #36393E; color: #FFFFFF'};} .katex { font-size: 1.1em; }</style>` + decoded_solution
