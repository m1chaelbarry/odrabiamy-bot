...
export default function getExerciseImage(solution: string, excercise_number: string, page_number: string, book_name: string): Promise<Buffer | null> {
    return new Promise(async (resolve) => {
        ...
        decoded_solution = `<style>html * {${decoded_solution.includes('<math-expr') ? '' : 'background: #36393E; color: #FFFFFF; font-family: MulishVariable,sans-serif;'}</style>` + decoded_solution
